function [u1,u2,u3,u4,y1,y2,y3,yzad] = zad6_DMC_klasyczny()

% Załadowanie odpowiedzi skokowych:
s11=load("odp_skok_y1_dla_u1.txt");
s12=load('odp_skok_y1_dla_u2.txt');
s13=load("odp_skok_y1_dla_u3.txt");
s14=load("odp_skok_y1_dla_u4.txt");
s21=load('odp_skok_y2_dla_u1.txt');
s22=load('odp_skok_y2_dla_u2.txt');
s23=load("odp_skok_y2_dla_u3.txt");
s24=load("odp_skok_y2_dla_u4.txt");
s31=load("odp_skok_y3_dla_u1.txt");
s32=load("odp_skok_y3_dla_u2.txt");
s33=load("odp_skok_y3_dla_u3.txt");
s34=load("odp_skok_y3_dla_u4.txt");

% Inicjalizacja parametrów DMC oraz błędu:
D=300;
N=300; %300, 150, 50, 25, 10, 1
Nu=300; %300, 150, 50, 25, 10, 1
E=0;

% Inicjalizacja zmiennych ilościowych (liczba wejść i wyjść):
nu=4;
ny=3;

% Stworzenie macierzy odpowiedzi skokowych:
S=cell(1,D);
for i=1:D
    S{i}=[s11(i) s12(i) s13(i) s14(i);
        s21(i) s22(i) s23(i) s24(i);
        s31(i) s32(i) s33(i) s34(i)];
end

% Tworzenie macierzy M:
M=cell(N,Nu);
for i=1:N
    for j=1:Nu
        if (i>=j)
            M{i,j}=S{min(i-j+1,D)};
        else
            M{i,j}=zeros(ny,nu);
        end
    end
end

% Tworzenie macierzy Mp:
Mp = cell(N, D-1);
for i=1:N
    for j=1:D-1
        if i + j <= D
            Mp{i,j}=S{i+j}-S{j};
        else
            Mp{i,j}=S{D}-S{j};
        end
    end
end

% Tworzenie macierzy lambda:
wektor_lambda=[1 1 1 1];
lambda=eye(nu);
for i=1:nu
    for j=1:nu
        if i==j
            lambda(i,j)=wektor_lambda(i);
        end
    end
end
Lambda=cell(Nu,Nu);
for i=1:Nu
    for j=1:Nu
        if i==j
            Lambda{i,j}=lambda;
        else
            Lambda{i,j}=zeros(nu,nu);
        end
    end
end

% Tworzenie macierzy psi:
wektor_psi=[1 1 1];
psi=eye(ny);
for i=1:ny
    for j=1:ny
        if i==j
            psi(i,j)=wektor_psi(i);
        end
    end
end
Psi=cell(N,N);
for i=1:N
    for j=1:N
        if i==j
            Psi{i,j}=psi;
        else
            Psi{i,j}=zeros(ny,ny);
        end
    end
end

% Konwersja obliczonych powyżej macierzy:
M=cell2mat(M);
Mp=cell2mat(Mp);
Psi=cell2mat(Psi);
Lambda=cell2mat(Lambda);

% Obliczenie macierzy K:
K=(M'*Psi*M+Lambda)^(-1)*M'*Psi;

% Określenie warunków początkowych:
kk=1000;
y1(1:kk)=0;
y2(1:kk)=0;
y3(1:kk)=0;
u1(1:kk)=0;
u2(1:kk)=0;
u3(1:kk)=0;
u4(1:kk)=0;

% Określenie trajektorii zadanej:
yzad(1:kk)=0;
yzad(100:kk)=2;
yzad(400:kk)=-6;
yzad(700:kk)=5;

% Algorytm DMC:
for k=5:kk
    % Pomiar wyjścia:
    [y1(k),y2(k),y3(k)]=symulacja_obiektu15y_p4( ...
        u1(k-1),u1(k-2),u1(k-3),u1(k-4), ...
        u2(k-1),u2(k-2),u2(k-3),u2(k-4), ...
        u3(k-1),u3(k-2),u3(k-3),u3(k-4), ...
        u4(k-1),u4(k-2),u4(k-3),u4(k-4), ...
        y1(k-1),y1(k-2),y1(k-3),y1(k-4), ...
        y2(k-1),y2(k-2),y2(k-3),y2(k-4), ...
        y3(k-1),y3(k-2),y3(k-3),y3(k-4));

    % Tworzenie wektorów Y, Yzad oraz wyliczenie wektora dUp:
    Y=cell(N,1);
    Yzad=cell(N,1);
    dUp=cell(D-1,1);
    for i=1:D-1
        dUp{i}=[0;0;0;0];
        if k-i-1>0
            dUp{i}=[u1(k-i)-u1(k-1-i);u2(k-i)-u2(k-1-i);u3(k-i)-u3(k-1-i);u4(k-i)-u4(k-1-i)];
        end
    end
    for i=1:N
        Y{i}=[y1(k) y2(k) y3(k)]';
    end
    for i=1:N
        Yzad{i}=[yzad(k) yzad(k) yzad(k)]';
    end

    % Konwersja obliczonych powyżej wektorów::
    Y=cell2mat(Y);
    Yzad=cell2mat(Yzad);
    dUp=cell2mat(dUp);
    % Wyliczenie wektora przyrostów sterowań:
    dU = K * (Yzad - Y - Mp * dUp);
    % Obliczenie poszczególnych sterowań:
    u1(k)=u1(k-1)+dU(1);
    u2(k)=u2(k-1)+dU(2);
    u3(k)=u3(k-1)+dU(3);
    u4(k)=u4(k-1)+dU(4);
    % Wyliczenie błędu:
    E=E+(yzad(k)-y1(k))^2+(yzad(k)-y2(k))^2+(yzad(k)-y3(k))^2;
end